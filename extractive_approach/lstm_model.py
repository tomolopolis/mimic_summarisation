...
class LSTMClf(nn.Module):
    def __init__(self, hidden_dim=100, bidirectional=True):
        super(LSTMClf, self).__init__()
        self.bidirectional = bidirectional
        self.hidden_dim = hidden_dim
        self.model = LSTM(384, hidden_dim, num_layers=2, batch_first=True, dropout=0.5, bidirectional=bidirectional)
        if bidirectional:
            hidden_dim *= 2
...
class MimicHospCourseDataset(Dataset):
    def __init__(self, split, pred_lim, size_lim=-1):
        ...
        columns_to_keep = ['text_embed_limd', 'text_embed_len', 
...
#         begin = time.time()
        item = self.ds[idx]
        tup = (torch.tensor(item['text_embed_limd']), \
               item['text_embed_len'], \
